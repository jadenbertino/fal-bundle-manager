...
class TestIntegrationScenarios:
    ...
    def create_test_files(self, base_dir: Path) -> dict:
        ...
        files["dir_file1"] = {"path": str(file1), "content": "Content 1", "size": 9}

        file2 = test_dir / "file2.txt"
        file2.write_text("Content 2")
        files["dir_file2"] = {"path": str(file2), "content": "Content 2", "size": 9}
        ...
        empty_file.touch()
        files["empty"] = {"path": str(empty_file), "content": "", "size": 0}
            ...
            expected_merkle = self.compute_expected_merkle_root(
                {
                    ...
                    "data": {
                        "path": str(test_dir / "data.json"),
                        "content": '{"key": "value"}',
                        "size": 16,
                    },
                    "model": {
                        "path": str(models_dir / "model.bin"),
    ...
    def test_mixed_files_and_directories(
        self, mock_client_class, runner, mock_api_client
    ):
        ...
        with runner.isolated_filesystem():
            ...
            blobs = [
                Blob(
                    ...
                    size_bytes=8,
                    hash=hashlib.sha256(b"content1").hexdigest(),
                    hash_algo="sha256",
                ),
                Blob(
                    bundle_path="file2.txt",
                    size_bytes=8,
                    hash=hashlib.sha256(b"content2").hexdigest(),
                    hash_algo="sha256",
                ),
            ]
        ...
        with runner.isolated_filesystem():
            ...
            with patch("cli.commands.create.BundlesAPIClient") as mock_client_class:
                ...
                result = runner.invoke(
                    cli, ["create", "file with spaces.txt", "file_ñ_中文.txt"]
                )
                assert result.exit_code == 0
    ...
    def test_download_verification(self, runner):
        ...
        with runner.isolated_filesystem():
            ...
            with (
                patch("cli.commands.create.BundlesAPIClient") as mock_create_client,
        ...
        with runner.isolated_filesystem():
            ...
            blobs = [
                Blob(
                    bundle_path="file1.txt",
                    size_bytes=9,
                    hash=hashlib.sha256(b"Content 1").hexdigest(),
                    hash_algo="sha256",
                ),
                Blob(
                    bundle_path="file2.txt",
                    size_bytes=9,
                    hash=hashlib.sha256(b"Content 2").hexdigest(),
                    hash_algo="sha256",
                ),
            ]
            expected_merkle = compute_merkle_root(blobs)
            ...
            with (
                ...
                assert recomputed_merkle == expected_merkle, (
                    f"Merkle root mismatch: expected {expected_merkle}, got {recomputed_merkle}"
                )
        ...
        with runner.isolated_filesystem():
            ...
            expected_blobs = [
                Blob(
                    ...
                    size_bytes=35,
                    hash=hashlib.sha256(b"# Test Project\nThis is a test.").hexdigest(),
                    hash_algo="sha256",
                ),
                Blob(
                    bundle_path="config.yaml",
                    size_bytes=25,
                    hash=hashlib.sha256(b"app: test\nversion: 1.0").hexdigest(),
                    hash_algo="sha256",
                ),
                Blob(
                    bundle_path="models/model.bin",
                    size_bytes=18,
                    hash=hashlib.sha256(b"binary_model_data").hexdigest(),
                    hash_algo="sha256",
                ),
                Blob(
                    bundle_path="data/dataset.json",
                    size_bytes=20,
                    hash=hashlib.sha256(b'{"samples": 100}').hexdigest(),
                    hash_algo="sha256",
                ),
            ]
            ...
            with (
                ...
                for file_path, expected_content in extracted_files.items():
                    ...
                    assert full_path.exists(), (
                        f"File {file_path} not found in extracted bundle"
                    )
                ...
                for i, (extracted_blob, expected_blob) in enumerate(
                    zip(extracted_blobs, expected_blobs)
                ):
                    assert extracted_blob.hash == expected_blob.hash, (
                        f"Hash mismatch for file {i}: {extracted_blob.bundle_path}"
                    )
                    assert extracted_blob.size_bytes == expected_blob.size_bytes, (
                        f"Size mismatch for file {i}: {extracted_blob.bundle_path}"
                    )
