...
class TestCreateSuccess:
    ...
    @patch("cli.commands.create.BundlesAPIClient")
    def test_create_with_single_directory(
    ...
    @patch("cli.commands.create.BundlesAPIClient")
    def test_create_with_multiple_paths(
        self, mock_client_class, runner, mock_api_client
    ):
        ...
        result = runner.invoke(
            cli, ["create", str(file1), str(configs_dir), str(file2)]
        )
...
class TestCreateNetworkErrors:
    ...
    def test_create_with_connection_error(self, mock_client_class, runner):
        ...
        mock_client = Mock()
        mock_client.preflight.side_effect = requests.exceptions.ConnectionError(
            "Connection failed"
        )
        ...
        assert (
            "failed to connect" in result.output.lower()
    ...
    def test_create_with_timeout_error(self, mock_client_class, runner):
        ...
        mock_client = Mock()
        mock_client.preflight.side_effect = requests.exceptions.Timeout(
            "Request timed out"
        )
        mock_client_class.return_value = mock_client
    ...
    def test_create_with_http_error(self, mock_client_class, runner):
        ...
        mock_client = Mock()
        mock_client.preflight.side_effect = requests.exceptions.HTTPError(
            "500 Server Error"
        )
        mock_client_class.return_value = mock_client
